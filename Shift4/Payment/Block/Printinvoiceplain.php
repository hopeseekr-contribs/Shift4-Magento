...
class Printinvoiceplain extends \Magento\Sales\Block\Items\AbstractItems
{
    ...
    public function getTotalsHtml()
    {
        ...
        if ($this->invoice) {
            $totals = $this->getLayout()
                ...
                ->setTemplate('Shift4_Payment::totals_plain.phtml');
        } else {
            $totals = $this->getLayout()
    ...
    public function itemRenderer($type, $item)
    {
        ...
        $typeBlock = $this->getLayout()
            ...
            ->setItem($item)->setTemplate('Shift4_Payment::renderer_plain.phtml');

        $rendererList = $this->getLayout()
            ->createBlock(\Magento\Framework\View\Element\RendererList::class)->setChild($type, $typeBlock);
