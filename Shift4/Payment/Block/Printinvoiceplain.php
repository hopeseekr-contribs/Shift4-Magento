...
class Printinvoiceplain extends \Magento\Sales\Block\Items\AbstractItems
{
    ...
    public function __construct(
        ...
        $this->getLayout()->createBlock('Magento\Tax\Block\Item\Price\Renderer', 'item_unit_price')->setTemplate('Shift4_Payment::unit_plain.phtml');
        $this->getLayout()->createBlock('Magento\Tax\Block\Item\Price\Renderer', 'item_row_total')->setTemplate('Shift4_Payment::row_plain.phtml');
        $this->getLayout()->createBlock('Magento\Tax\Block\Item\Price\Renderer', 'item_row_total_after_discount')->setTemplate('Shift4_Payment::total_after_discount_plain.phtml');
    }
    ...
    {
        $this->setTemplate('Shift4_Payment::invoice_plain.phtml');
    ...
    public function getTotalsHtml()
    {
        ...
        if ($this->invoice) {
            $totals = $this->getLayout()->createBlock('Magento\Sales\Block\Order\Invoice\Totals', 'invoice_totals')->setTemplate('Shift4_Payment::totals_plain.phtml');
        } else {
            $totals = $this->getLayout()->createBlock('Magento\Sales\Block\Order\Totals', 'invoice_totals')->setTemplate('Shift4_Payment::totals_plain.phtml');
        }
        
        $this->setChild('invoice_totals', $totals);

        $customerbalance = $this->getLayout()->createBlock('Magento\CustomerBalance\Block\Sales\Order\Customerbalance', 'customerbalance')->setTemplate('Shift4_Payment::customerbalance_plain.phtml');
        $totals->setChild('customerbalance', $customerbalance);
        
        $tax = $this->getLayout()->createBlock('Magento\Tax\Block\Sales\Order\Tax', 'tax')->setTemplate('Shift4_Payment::tax_plain.phtml');
    ...
    public function itemRenderer($type, $item)
    {
        ...
        $typeBlock = $this->getLayout()->createBlock('Magento\Sales\Block\Order\Item\Renderer\DefaultRenderer')->setItem($item)->setTemplate('Shift4_Payment::renderer_plain.phtml');
