    ...
    try {
        ...
        if (_wallets_canMakeApplePayments) {
            ...
            session.onvalidatemerchant = function (event) {
                ...
                getApplePaySession(validationURL).then(
                    function (response) {
                        ...
                        if (_wallets_i4goTrueTokenObj.settings.wallet.shippingOptionRequired) {
                             session.onshippingcontactselected = function (event) {
                                apOnShippingContactSelected(event, session); }
                            session.onshippingmethodselected = function (event) {
                                apOnShippingMethodSelected(event, session); }
                        }
                    }, function (error) {
    ...
    try {
        return new Promise(
            function (resolve, reject) {
                ...
                _wallets_i4goTrueTokenObj.settings.debug && remoteLog(
                    "Sending getApplePaySession: " + JSON.stringify(
                        {
                            url: url,
                            data: data
                        }
                ...
                jQuery.post(
                    url, data, function (responseData, status) {
                        if (status == "success") {
                            resolve(responseData);
                        } else {
                            _wallets_i4goTrueTokenObj.settings.debug && remoteLog(
                                "Failed getApplePaySession: " + JSON.stringify(
                                    {
                                        url: url,
...
function apGetDefaultShippingMethods()
{
    ...
    if (typeof _wallets_i4goTrueTokenObj.settings.wallet.shippingOptions === "object") {
        _wallets_i4goTrueTokenObj.settings.wallet.shippingOptions.forEach(
            function (item) {
...
function gp2apPaymentDataRequestUpdate(gpPaymentDataRequestUpdate)
{
    ...
    gpPaymentDataRequestUpdate.newTransactionInfo.displayItems.forEach(
        function (item) {
    ...
    if (typeof gpPaymentDataRequestUpdate.newShippingOptionParameters === "object") {
        try {
            ...
            gpPaymentDataRequestUpdate.newShippingOptionParameters.shippingOptions.forEach(
                function (item) {
                    let amount = "0.00";
...
function apOnShippingContactSelected(event, session)
{
    ...
    .then(
        function (paymentDataRequestUpdate) {
            session.completeShippingContactSelection(paymentDataRequestUpdate)}
    )
    .catch(
        function (e) {
...
function apOnShippingMethodSelected(event, session)
{
    ...
    .then(
        function (paymentDataRequestUpdate) {
            session.completeShippingMethodSelection(paymentDataRequestUpdate)}
    )
    .catch(
        function (e) {
...
function googlePayInit(config)
{
    try {
        ...
        if ((typeof config === "object") 
            ...
            baseCardPaymentMethod.parameters = Object.assign(
                baseCardPaymentMethod.parameters, {
...
function onGooglePayLoaded()
{
    ...
    paymentsClient.isReadyToPay(getGoogleIsReadyToPayRequest()).then(
        function (response) {
...
function getGoogleTransactionInfo()
{
    try {
        ...
        if (typeof _wallets_i4goTrueTokenObj.basket.Cart === "object" && _wallets_i4goTrueTokenObj.basket.Cart.length > 0) {
            _wallets_i4goTrueTokenObj.basket.Cart.forEach(
                function (item) {
